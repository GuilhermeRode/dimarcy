...
def gerar(pedido: Pedido, path: str):
    ...
    vendedor_line = ""
    if pedido.vendedor:
        vendedor_line = f'<br/><font color="#888780">Vendedor: </font><b>{pedido.vendedor}</b>'
    ...
    tot = Table([
        ["Total de peças", str(pedido.total_pecas),
         "Valor bruto", f"R$ {pedido.valor_bruto:.2f}",
         "Forma de pagamento", pedido.forma_pgto or "—"],
        [f"Desconto", f"{pedido.desconto:.1f}%",
         "Valor líquido", f"R$ {pedido.valor_liquido:.2f}",
    ...
    pg = [["Tipo de entrega", pedido.tipo_entrega or "—",
...
def gerar_producao(producao: dict, path: str):
    """
    producao = {
        "numero": str,
        "data":   str,
        "obs":    str,
        "itens": [{"referencia","npk","cor","pp","p","m","g","gg","xgg","unico"}, ...]
    }
    """
    cfg     = config_model.get_all()
    empresa = cfg.get("empresa_nome", "Empresa")
    doc     = SimpleDocTemplate(
        ...
        leftMargin=2*cm, rightMargin=2*cm,
        topMargin=2*cm, bottomMargin=2*cm,
        title=f"Produção {producao.get('numero','')}",
        author=empresa
    )
    s_sec    = _ps("sec2", fontName="Helvetica-Bold", fontSize=10,
                   textColor=_VERDE, spaceBefore=14, spaceAfter=5)
    s_footer = _ps("f2", fontName="Helvetica", fontSize=7.5,
                   textColor=_CINZA, alignment=TA_CENTER)
    story = []
    ...
    hd = Table([[
        Paragraph(f"<b>{empresa}</b>",
                  _ps("h2", fontName="Helvetica-Bold", fontSize=14, textColor=_VERDE)),
        Paragraph(
            f"<font color='#888780'>Produção</font> <b>{producao.get('numero','')}</b><br/>"
            f"<font color='#888780'>Data: {producao.get('data','—')}</font>",
            _ps("hr2", fontName="Helvetica", fontSize=9,
                textColor=_AZUL_ESC, alignment=TA_RIGHT)
        ),
    ]], colWidths=[10*cm, 7*cm])
    hd.setStyle(TableStyle([
    ...
    story.append(hd)
    story.append(HRFlowable(width="100%", thickness=1.5, color=_VERDE, spaceAfter=12))

    story.append(Paragraph("ITENS DE PRODUÇÃO", s_sec))

    sz_labels = ["PP", "P", "M", "G", "GG", "XGG", "Único"]
    sz_keys   = ["pp", "p", "m", "g", "gg", "xgg", "unico"]
    header = ["Ref.", "NPK", "Cor"] + sz_labels + ["Total"]
    rows = [header]
    for it in producao.get("itens", []):
        qtds = [it.get(k, 0) for k in sz_keys]
        total = sum(q for q in qtds if q)
        rows.append([
            it.get("referencia", ""),
            it.get("npk", ""),
            it.get("cor", ""),
    ...
    col_w = [2*cm, 2*cm, 3*cm] + [1.1*cm]*7 + [1.5*cm]
    ...
    t.setStyle(TableStyle([
        ...
        ("FONTNAME", (0,1), (-1,-1), "Helvetica"),
        ("FONTSIZE", (0,0), (-1,-1), 8),
        ("ROWBACKGROUNDS", (0,1), (-1,-1), [_BRANCO, colors.HexColor("#E3F7ED")]),
        ("GRID", (0,0), (-1,-1), 0.3, _CINZA_CLR),
        ("ALIGN", (3,0), (-1,-1), "CENTER"),
        ("PADDING", (0,0), (-1,-1), 5),
        ("FONTNAME", (-1,1), (-1,-1), "Helvetica-Bold"),
    ...
    if producao.get("obs"):
        ...
        story.append(Paragraph("OBSERVAÇÕES", s_sec))
        story.append(Paragraph(producao["obs"],
                                _ps("obs2", fontName="Helvetica", fontSize=9)))
