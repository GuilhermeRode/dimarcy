...
# Paleta
MARROM    = colors.HexColor("#3D1A08")
CARAMELO  = colors.HexColor("#C8813A")
CREME     = colors.HexColor("#FDF6EE")
...
def gerar(pedido: Pedido, path: str):
    ...
    s_sec    = _ps("sec", fontName="Helvetica-Bold", fontSize=10,
                   textColor=MARROM, spaceBefore=14, spaceAfter=5)
    ...
    # ── Cabeçalho ────────────────────────────────────────────────────────────
    header_data = [[
        Paragraph(f"<b>{empresa}</b>", _ps("h", fontName="Helvetica-Bold",
                  fontSize=18, textColor=MARROM)),
        Paragraph(
    ...
    col_hdrs = ["Ref.", "Descrição", "Cor", "Mat."] + TAMANHOS + ["Total", "Unit.", "Subtotal"]
    rows = [col_hdrs]
    ...
    tot_data = [
        ...
        [f"Desconto ({pedido.desconto:.1f}%)",
         f"- R$ {pedido.valor_bruto - pedido.valor_liquido:.2f}",
         "Valor líquido", f"R$ {pedido.valor_liquido:.2f}"],
        ["Entrada paga", f"R$ {pedido.entrada:.2f}",
         "Saldo a pagar", f"R$ {pedido.saldo:.2f}"],
    ]
    tt = Table(tot_data, colWidths=[3.5*cm, 2.8*cm, 3.5*cm, 3.2*cm])
    tt.setStyle(TableStyle([
        ...
        ("GRID",       (0,0), (-1,-1), 0.4, colors.HexColor("#E8C99A")),
        ("PADDING",    (0,0), (-1,-1), 6),
        ("FONTNAME",   (3,2), (3,2), "Helvetica-Bold"),
        ("TEXTCOLOR",  (3,2), (3,2), MARROM),
        ("FONTSIZE",   (3,2), (3,2), 10),
    ]))
    story.append(tt)
    ...
    pgt = Table(pg_data, colWidths=[3*cm, 6*cm, 2.8*cm, 5.2*cm])
