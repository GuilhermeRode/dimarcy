...
class FormPedidoView(tk.Frame):
    ...
    def _build_cliente(self, parent):
        ...
        nome_entry = self._lbl_entry(r1[0], "Nome / razão social *", self._v["cli_nome"])
        nome_entry.bind("<KeyRelease>", self._sugerir_cliente)
        self._sugestoes = tk.Listbox(r1[0], height=4, font=FONT_SMALL)
        self._sugestoes.bind("<<ListboxSelect>>", self._selecionar_sugestao_cliente)
    ...
    def prefill_cliente(self, cliente: dict):
        self._v["cli_nome"].set(cliente.get("cliente_nome", ""))
        self._v["cli_doc"].set(cliente.get("cliente_doc", ""))
        self._v["cli_tel"].set(cliente.get("cliente_tel", ""))
        self._v["cli_email"].set(cliente.get("cliente_email", ""))
        self._v["cli_cidade"].set(cliente.get("cliente_cidade", ""))

    def _sugerir_cliente(self, _event=None):
        termo = self._v["cli_nome"].get().strip().lower()
        self._sugestoes.delete(0, "end")
        if not termo:
            self._sugestoes.pack_forget()
            return
        self._clientes_cache = self.ctrl.clientes()
        hits = [c for c in self._clientes_cache if termo in str(c.get("cliente_nome", "")).lower()][:8]
        if not hits:
            self._sugestoes.pack_forget()
            return
        for c in hits:
            self._sugestoes.insert("end", c.get("cliente_nome", ""))
        self._sugestoes.pack(fill="x", pady=(2,0))

    def _selecionar_sugestao_cliente(self, _event=None):
        if not self._sugestoes.curselection():
            return
        nome = self._sugestoes.get(self._sugestoes.curselection()[0])
        for c in getattr(self, "_clientes_cache", []):
            if c.get("cliente_nome") == nome:
                self.prefill_cliente(c)
                break
        self._sugestoes.pack_forget()
...
class _ItemRow(tk.Frame):
    ...
    def _selecionar_cor(self, nome_cor):
        self._vs["cor"].set(nome_cor)
    ...
    def _build(self, idx: int):
        ...
        for i, (lbl, key, weight) in enumerate(fields):
         ...
         # Cores disponíveis da referência (estilo vitrine)
        self._cores_paleta = tk.Frame(body, bg="#F7F1EA", highlightbackground="#E6DACB", highlightthickness=1)
        self._cores_paleta.pack(fill="x", pady=(0, 8))
        tk.Label(self._cores_paleta, text="Cores disponíveis", font=(FONT_FAMILY, 9, "bold"), fg="#7A5A3A", bg="#F7F1EA").pack(side="left", padx=(8, 8), pady=6)
    ...
    def _render_cores(self, cores: list[str]):
        ...
        for nome_cor in cores:
            ...
            cor_hex = mapa.get(nome_cor.lower(), "#607D8B")
            ...
            cv.bind(
                "<Enter>",
                lambda e, nome=nome_cor: self._show_color_tip(e, nome)
            )

            cv.bind("<Leave>", self._hide_color_tip)

    def _show_color_tip(self, event, nome_cor: str):

        self._hide_color_tip()
    ...
    def _hide_color_tip(self, _event=None):
...
PRODUTOS_MOCK = {
    "1130": {
        ...
        "preco": 109.8,
        "cores": ["preto", "marinho", "vinho", "raiz", "ninive"]
        },

    "1163": {
        "descricao": "Calça sweet friso",
        "preco": 139.80,
        "cores": ["preto", "bege", "bosque", "raiz", "vinho"]
    },
