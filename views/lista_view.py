"""
views/lista_view.py — Lista de pedidos com busca, filtros e ações rápidas
"""
import tkinter as tk
from tkinter import ttk
from views.theme import *
from views.widgets import PrimaryBtn, SecondaryBtn, DangerBtn, build_treeview

STATUS_OPTS = ["Todos","Rascunho","Confirmado","Em produção",
...
class ListaView(tk.Frame):
    ...
    def _build(self):
        ...
        # Treeview — todas as colunas centralizadas
    ...
    def _abrir(self):
        pid = self._sel_id()
        if pid: self.app.abrir_pedido(pid)

    def _excluir(self):
        pid = self._sel_id()
        if pid and self.ctrl.excluir(pid): self.refresh()

    def _pdf(self):
        pid = self._sel_id()
        if not pid: return
        p = self.ctrl.carregar(pid)
        if p: self.ctrl.exportar_pdf(p)

    def _change_status(self, _):
        pid  = self._sel_id()
        novo = self._novo_status.get()
        if not pid or novo == "Status": return
        from models import pedido_model
        p = pedido_model.buscar(pid)
        if p:
            p.status = novo
            pedido_model.salvar(p)
            self.refresh()
